...
import glob
import socket
from Code.files import give as file1
...
class scrape:
    ...
    def json(self):
        try:
            pd.read_excel("Files\\"+str(self.identifier)+"-"+self.identifier_value+" "+str(datetime.datetime.now().strftime("[%H.%M] [%d-%m-%Y]"))+".xlsx").to_json("files\\"+identifier+"-"+identifier_value+" "+str(datetime.datetime.now().strftime("[%H.%M] [%d-%m-%Y]"))+".json")
        except:
            get_file3=file1()
            file = get_file3.latest_one_file()[0]
